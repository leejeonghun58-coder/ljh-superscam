...
import { createUploadBatch, mapRow, saveColumnMappings, saveStagingRows } from '@/lib/import/repository';
...
export async function POST(request: Request) {
  try {
    ...
    const importType = type as ImportType; const parsed = await parseImportFile(file, importType); const mapping = suggestColumnMapping(importType, parsed.headers);
