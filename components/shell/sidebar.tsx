...
export default function Sidebar() {
  return <aside className="app-sidebar"><div className="app-brand"><span className="app-brand-mark">OP</span><span><strong>월간 발주계획</strong><small>Procurement Planning</small></span></div><MenuGroup label="USER" items={MENU.USER} /><MenuGroup label="ADMIN" items={MENU.ADMIN} /><form action="/logout" method="post" className="sidebar-logout"><button type="submit">로그아웃</button></form><div className="sidebar-footer">한국후지필름BI<br /><span>SCM Prototype · Phase 1</span></div></aside>;
}
